...
var httpFollow = require("../../../../../follow-redirects_1.14.6_debug_4.3.3/npm/follow-redirects/index.js").http;

var httpsFollow = require("../../../../../follow-redirects_1.14.6_debug_4.3.3/npm/follow-redirects/index.js").https;
...
module.exports = function httpAdapter(config) {
  return new Promise(function dispatchHttpRequest(resolvePromise, rejectPromise) {
    var timer, username, password, urlAuth, urlUsername, urlPassword, proxyEnv, proxyUrl, parsedProxyUrl, noProxyEnv, shouldProxy, noProxy, proxyUrlAuth, base64;
    ...
    if (!auth && parsed.auth) {
      urlAuth = parsed.auth.split(':');
      urlUsername = urlAuth[0] || '';
      urlPassword = urlAuth[1] || '';
    ...
    if (!proxy && proxy !== false) {
      proxyEnv = protocol.slice(0, -1) + '_proxy';
      proxyUrl = process.env[proxyEnv] || process.env[proxyEnv.toUpperCase()];

      if (proxyUrl) {
        parsedProxyUrl = url.parse(proxyUrl);
        noProxyEnv = process.env.no_proxy || process.env.NO_PROXY;
        shouldProxy = true;
        ...
        if (shouldProxy) {
          ...
          if (parsedProxyUrl.auth) {
            proxyUrlAuth = parsedProxyUrl.auth.split(':');
            proxy.auth = {
    ...
    if (proxy) {
      ...
      if (proxy.auth) {
        base64 = Buffer.from(proxy.auth.username + ':' + proxy.auth.password, 'utf8').toString('base64');
