...
RedirectableRequest.prototype._sanitizeOptions = function (options) {
  ...
  if (!options.pathname && options.path) {
    searchPos = options.path.indexOf("?");
...
RedirectableRequest.prototype._performRequest = function () {
  ...
  if (this._isRedirect) {
    ...
    i = 0;
    self = this;
    buffers = this._requestBodyBuffers;
...
RedirectableRequest.prototype._processResponse = function (response) {
  ...
  if (location && this._options.followRedirects !== false && statusCode >= 300 && statusCode < 400) {
    ...
    currentHostHeader = removeMatchingHeaders(/^host$/i, this._options.headers); // If the redirect is relative, carry over the host of the last request

    currentUrlParts = url.parse(this._currentUrl);
    currentHost = currentHostHeader || currentUrlParts.host;
    currentUrl = /^\w+:/.test(location) ? this._currentUrl : url.format(Object.assign(currentUrlParts, {
      host: currentHost
    ...
    redirectUrlParts = url.parse(redirectUrl);
...
function wrap(protocols) {
  ...
  Object.keys(protocols).forEach(function (scheme) {
    ...
    Object.defineProperties(wrappedProtocol, {
      ...
      get: {
        value: function get(input, options, callback) {
